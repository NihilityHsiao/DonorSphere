 ...
 */
    ...
    */
    ...
    /**
     * Modifier: onlyNotRegisteredNGO
     * 
     * Description: Checks if the NGO with the given registration number is not already registered.
     * 
     * Parameters:
     @param _ngoRegisterationNO: The registration number of the NGO to check.
     * 
     * Requirements:
     * - The NGO with the given registration number must not be already registered.
     * 
     * Error:
     * - NGO_ALREADY_REGISTERED: The NGO with the given registration number is already registered.
     */
    // Modifier to check if an NGO is not already registered
    modifier onlyNotRegisteredNGO(uint256 _ngoRegisterationNO) {
        require(!registeredNGOs[_ngoRegisterationNO], "NGO_ALREADY_REGISTERED");
        _;
    }
    ...
    Checks if the caller is a DAO Token holder.

    Requirements:
    - The caller must have a balance of 1 DAO Token.

    Usage:
    - Apply this modifier to functions that should only be accessible by DAO Token holders.

    */
    //Modifier for only DAO Token Holder to vote
    modifier daoTokenHolderOnly() {
        require(
            _token_Contract.balanceOf(msg.sender) >= 1,
     ...
     * 
     * Checks if the NGO with the given registration number is registered.
     * 
     * Parameters:
     * - _ngoRegisterationNO: The registration number of the NGO to check.
     * 
     * Reverts:
     * - NGO_NOT_REGISTERED: If the NGO with the given registration number is not registered.
     */
    //Modifier to check NGO is registered
    modifier onlyRegisteredNGO(uint256 _ngoRegisterationNO) {
        require(registeredNGOs[_ngoRegisterationNO], "NGO_NOT_REGISTERED");
        _;
    }

    modifier notRegisteredNGO(uint256 _ngoRegisterationNO) {
        require(!(registeredNGOs[_ngoRegisterationNO]), "NGORegistered");
        _;
    }
    /**
    ...
    * @param _tokenId :The ID of the campaign to vote against.
    ...
    function updateTotalBeneficary(
        ...
        ngototalBeneficiary[_ngoRegisterationNO] = _noOfBeneficiary;
        return true;
    }
    ...
    */
    ...
    ) external onlyRegisteredNGO(_ngoRegisterationNO) returns (uint256) {
        require(
            ngoOwner[_ngoRegisterationNO] == msg.sender,
    ...
    @param _ngoAddress: The address of the NGO to be registered.
    @param _ngoRegisterationNO: The registration number of the NGO.

    */
    function registerationForNGO(
        address _ngoAddress,
        uint256 _ngoRegisterationNO
    ) external onlyNotRegisteredNGO(_ngoRegisterationNO) {
        ngoRegistrationNo[_ngoAddress] = _ngoRegisterationNO;
        ngoOwner[_ngoRegisterationNO] = _ngoAddress;
        registeredNGOs[_ngoRegisterationNO] = false;
        emit registerationNGO(_ngoAddress, _ngoRegisterationNO);
    }
    ...
    */
    function voteForNGO(
        ...
        external
        notRegisteredNGO(_ngoRegisterationNO)
    ...
    */
    ...
    function claimFundsOfBeneficiary(
        ...
        require(
            beneficiaryHasVoucherInCampaign[_beneficiary][_campaignID] == 1,
            "This beneficiary doesn't have voucher"
        );
        require(
            priceOfVoucher[_beneficiary][_campaignID] > 0,
            "This beneficiary doesn't have price"
        );

        beneficiaryHasVoucherInCampaign[_beneficiary][_campaignID] == 0;

        (bool sent, ) = payable(_vendor).call{
            value: priceOfVoucher[_beneficiary][_campaignID]
        }("");
        require(sent, "Failed to send Ether");
